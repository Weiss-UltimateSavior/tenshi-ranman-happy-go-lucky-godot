...
>
> **总状态(2026-09-10)**:§1(SCN JSON 转换器,216/216)、§2(素材归位)、
> §3-B(PBD 本地解码器,68/68,立绘实机验证通过)均**已完成**;
> §3.2 降级路线 C **已作废**(被本地解码器取代);仅 §3.1-A 批
> (2 个加密 `.scn`,日文文本)仍可选依赖 Windows——不阻塞任何主线功能。
...
### 3.2 降级路线(已作废)~~整身合成图 + `.stand` 偏移~~

> **状态:已作废,不再执行(2026-09-10)。**
> 该路线原为"B 批 PBD 无法本地解码"的退路。§6 记录的 `tools/pbd_to_json.py`
> 已用**纯 Python 本地解出全部 68 个 PBD 元数据**(与 .NET 参考实现逐字段
> 一致),立绘走 PBD 精确合成并已在实机验证(金发蓝眼、部件正确、不散架)。
> 降级路线因此没有存在意义;以下条目保留仅作历史记录,**不勾选**。
>
> 保留的技术价值:整身合成图 `<prefix>_<变体>_body.png` 已归位
> (`docs/fgimage_relocation.md`),可作为素材备用;其中"只用 `.stand` 已声明
> 偏移、不猜编号"的原则继续适用于任何立绘定位改动。

1. [ ] ~~用 `<prefix>_<变体>_body.png` 作为身体底图~~(已作废)
2. [ ] ~~表情脸用 `.sinfo` 层名 + `facexoff/faceyoff` 定位~~(已作废)
3. [ ] ~~trace/QA 标注 `stand_source: "composite"`~~(已作废)
4. [ ] ~~局限写进文档~~(已作废;局限本身已因 PBD 解码而消失)

### 3.3 工作量

Windows 侧 0.5 小时操作 + 回传验证 0.5 天(仅 A 批 2 个加密 scn 仍需要;
B 批 PBD 已由 §6 的本地解码器完成,无需 Windows)。降级路线已作废,不计入。
...
项目三(Windows 补缺)      ← 仅剩 A 批 2 个加密 scn;B 批已本地完成(§6)
   └─ A(2 个加密 scn)→ 分支落点补全;立绘已由本地 PBD 解码器解锁
```
...
4. Windows 批次(可选,仅 A 批):`0429_sel` 分支落点可继续播放;
   立绘已由本地 PBD 解码器实现精确合成,两条既有轨迹基线保持全绿
